...
mod tests {
    ...
    fn help_enumerates_exactly_cli_surface_v1() {
        ...
        for command in &headings {
            if let Err(error) = parse(strings(&[*command])) {
                assert!(
                    !error.starts_with("unknown command:"),
                    "help names a command that the shipped parser rejects: {command}"
                );
            }
        }
