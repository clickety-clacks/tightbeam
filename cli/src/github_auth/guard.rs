...
pub(super) struct ClassifiedTarget {
    pub(super) operation_class: &'static str,
...
pub(super) fn classify_command_with(
    text: &str,
    gh: &impl Gh,
    known_hosts: &[String],
    ...
    for command in executable_commands(text) {
        if overrides_projection(&command) {
            return Ok(Classification::ProjectionOverride);
        }
        ...
        if looks_like_gh_repo_call(&command) {
            let hostname = match gh_target_hostname(&command) {
                ...
                Err(SelectorError::Malformed) => return Ok(Classification::MalformedToolCall),
            };
            targets.push(ClassifiedTarget {
                operation_class: "gh",
                hostname,
                remote: None,
            });
        ...
        if verb == "remote"
            && !args
                .iter()
                .any(|arg| matches!(arg.as_str(), "update" | "get-url"))
        {
            continue;
        }
        ...
        if let Some(value) = operand {
            if is_named_remote(value) {
                if let Some(url) = gh.git_remote_url(value)? {
                    candidates.push(url);
                }
            } else {
                candidates.push(value.to_owned());
            }
        } else if matches!(verb, "fetch" | "pull" | "push") {
            if let Some(url) = gh.git_remote_url("origin")? {
                candidates.push(url);
            }
        } else if verb == "submodule" && submodule_network_operation(args) {
            candidates.extend(gh.git_submodule_urls()?);
        }

        for remote in candidates {
            if let Some(hostname) = recognized_remote_hostname(&remote, known_hosts) {
                targets.push(ClassifiedTarget {
                    operation_class: "git",
                    hostname,
                    remote: Some(ScrubbedRemote::new(&remote).0),
                });
            }
        }
    }
    ...
    if targets.is_empty() {
        ...
        Ok(Classification::Targets(targets))
    }
}

pub(super) fn might_need_hostname_index(text: &str) -> bool {
    executable_commands(text)
        .iter()
        .any(|command| git_operation(command).is_some())
}

#[derive(Debug)]
struct ScrubbedRemote(String);

impl ScrubbedRemote {
    fn new(value: &str) -> Self {
        Self(super::redact::scrub_detail(value))
    }
}

fn overrides_projection(command: &[String]) -> bool {
    command.iter().any(|word| {
        word.split_once('=')
            .is_some_and(|(name, _)| RESERVED.contains(&name))
    }) || command.windows(2).any(|pair| {
        pair[0] == "-u" && RESERVED.contains(&pair[1].as_str())
            || pair[0] == "--unset" && RESERVED.contains(&pair[1].as_str())
    }) || command.iter().any(|word| {
        word.strip_prefix("--unset=")
...
fn gh_target_hostname(command: &[String]) -> Result<String, SelectorError> {
    ...
    while index < command.len() {
        ...
        if matches!(word, "--hostname" | "-R" | "--repo") {
            index += 1;
            let value = command.get(index).ok_or(SelectorError::Malformed)?;
            if word == "--hostname" {
                explicit.push(
                    normalize_selector_hostname(value).map_err(|()| SelectorError::Malformed)?,
                );
            ...
            explicit
                .push(normalize_selector_hostname(value).map_err(|()| SelectorError::Malformed)?);
...
fn repo_selector_hostname(value: &str) -> Result<Option<String>, ()> {
    if value.contains("://") || value.contains('@') {
        return Ok(parse_remote_hostname(value));
    }
    let parts = value.split('/').collect::<Vec<_>>();
    if parts.len() >= 3 {
        Ok(Some(normalize_selector_hostname(parts[0])?))
    } else {
        Ok(None)
    }
}

fn normalize_selector_hostname(value: &str) -> Result<String, ()> {
    ...
    if value.is_empty()
        ...
        || value.contains("://")
        || value.split(':').count() > 2
        || value.split(':').next().is_some_and(|host| host.is_empty())
    {
        return Err(());
    }
...
fn parse_remote_hostname(remote: &str) -> Option<String> {
    let scp_url;
    let parseable = if remote.contains("://") {
        remote
    } else {
...
fn recognized_remote_hostname(remote: &str, known_hosts: &[String]) -> Option<String> {
    let hostname = parse_remote_hostname(remote)?;
    (hostname == "github.com" || known_hosts.iter().any(|known| known == &hostname))
        .then_some(hostname)
}
...
mod tests {
    ...
    #[test]
    fn classifier_distinguishes_conflicting_host_selectors_from_malformed_input() {
        let gh = FakeGh::new(true);
        assert_eq!(
            classify_command_with(
                "gh api --hostname github.com --repo ghe.example/acme/widget",
                &gh,
                &[],
            )
            .unwrap(),
