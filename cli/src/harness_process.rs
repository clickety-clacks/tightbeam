...
pub fn group(args: &[String]) -> Result<i32, String> {
    if args.len() != 2 || !matches!(args[0].as_str(), "status" | "kill") {
        return Err("usage: tightbeam harness-group <status|kill> <process-group-id>".into());
    }
...
mod tests {
    ...
    fn group_rejects_non_positive_ids() {
        assert!(group(&["status".into(), "0".into()]).is_err());
        assert!(group(&["kill".into(), "-1".into()]).is_err());
    }
}
