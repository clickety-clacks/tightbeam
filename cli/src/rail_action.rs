...
fn argv_matches(action: Action, argv: &[String]) -> bool {
    ...
    match action {
        ...
        Action::GitRestore if subcommand == "restore" => !restore_index_only(rest),
        _ => false,
    }
}

fn restore_index_only(args: &[String]) -> bool {
    ...
    while let Some(arg) = args.get(index) {
        match arg.as_str() {
            ...
            "--quiet" | "-q" | "--pathspec-file-nul" => {}
            value
                if value.starts_with("--source=") || value.starts_with("--pathspec-from-file=") => {
            }
...
// Shell substitutions execute even when their surrounding argument is not a
// command segment. Preserve their bodies for classification, while leaving
// ordinary quoted prose alone. The tokenizer below then handles the extracted
// body with the same argv rules as a top-level command.
fn command_substitutions(text: &str) -> Vec<String> {
    ...
    fn parenthesized(chars: &[char], start: usize) -> Option<(String, usize)> {
        let mut quote = Quote::None;
        let mut escaped = false;
        let mut depth = 1;
        let mut index = start;
        ...
        match quote {
            ...
            Quote::Double => match ch {
                ...
                '`' => {
                    ...
                    while let Some(inner) = chars.get(index) {
                        if *inner == '`' {
                            ...
                            substitutions.push(body.clone());
                            substitutions.extend(command_substitutions(&body));
                            index += 1;
                            break;
                        }
                        index += 1;
                ...
                '$' if chars.get(index + 1) == Some(&'(') => {
                    if let Some((body, next)) = parenthesized(&chars, index + 2) {
                        substitutions.push(body.clone());
                        substitutions.extend(command_substitutions(&body));
                        index = next;
                    } else {
...
fn without_heredoc_bodies(command: &str) -> String {
    ...
    for line in command.lines() {
        if let Some((delimiter, strip_tabs, quoted)) = pending.front() {
            ...
            if candidate == delimiter {
                ...
                kept.extend(command_substitutions(line));
            }
            continue;
        }
...
mod tests {
    ...
    #[test]
    fn executable_backtick_substitutions_cannot_bypass_closed_actions() {
        for (action, destructive) in [
            ...
            assert!(command_matches(action, &format!("echo `{destructive}`")));
            assert!(!command_matches(action, &format!("echo '{destructive}'")));
        }
    }

    #[test]
    fn unquoted_heredoc_substitutions_cannot_bypass_closed_actions() {
        for (action, destructive) in [
            (Action::GitStash, "git stash"),
            (Action::GitResetHard, "git reset --hard HEAD"),
            (Action::GitCleanForce, "git clean -xdf"),
            ...
            let unquoted = format!("cat <<REPORT\n$( {destructive} )\nREPORT");
            assert!(command_matches(action, &unquoted), "{unquoted}");

            let quoted = format!("cat <<'REPORT'\n$( {destructive} )\nREPORT");
            assert!(!command_matches(action, &quoted), "{quoted}");
        }
    }
    ...
    #[test]
    fn index_only_restore_preserves_nested_and_worktree_guards() {
        for command in [
            "git restore --staged -- path",
            "git restore -S path",
            "git restore --source HEAD --staged -- path",
            "git restore --staged -- --worktree",
            "sh -c 'git restore --staged -- path'",
        ] {
            assert!(!command_matches(Action::GitRestore, command), "{command}");
        }
        for command in [
            "git restore path",
            "git restore --staged --worktree path",
            "git restore -SW path",
            "git restore --source --staged path",
            "git restore -- --staged",
            "git restore --staged path; git restore other",
            "sh -c 'git restore --staged path; git restore other'",
            "echo $(git restore path)",
            "echo `git restore path`",
        ] {
            assert!(command_matches(Action::GitRestore, command), "{command}");
