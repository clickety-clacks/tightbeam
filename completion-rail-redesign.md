...
Amends `p3-observables-producers-v1.md` at THREE coupled sites (a partial edit leaves the
canon self-contradictory — fable B1): (i) invariant 6b clause (b) removed, clause (a) kept
(invariant 6 UNCHANGED); (ii) the `independent_verdict_kinds` fact-def parenthetical (L398)
— drop "R not commissioned by A's holder"; (iii) the §4A sanction prose (L437-442) — it
becomes the HOME of the independence-axis gate policy (fable B2; states the ARMS + that the
org selects via rule config — no value baked in, D4) and its "all closed by invariants 6, 6b"
sentence is corrected. Also amends `kungfu/agentic-engineering/rules/engineering.toml`: the
`completion-requires-review` rule (subject gate + REQUIRE axis, value in rule CONFIG). And
amends `lib/tightbeam/assignments.ex` — FOUR code-space changes (fable delta D1; a
READY-TO-MINT spec must not depend on unlisted substrate changes):
1. `commissioned_review_authors` — drop the opener filter (L264).
2. a new `is_producing_card` fact (derived `reviewsAssignmentId IS NULL`; extended by Q1's
   `--non-producing` flag).
3. `valid_commit_refs` (L1337-1352) — RELAX to accept `commit_refs` on `kind = "verdict"`
   for review-link verdicts, so a reviewer can record the reviewed sha (§4/F5 mechanism is
   refused by the current code, which allows commit_refs on `kind = "completion"` ONLY).
4. `valid_commit_refs`/`commit_ref_filing_allowed` — FORBID `commit_refs` on a `completion`
   attest of a NON-producing card (the §3a/F5 anti-poison closure; forbid chosen over flag —
   make the poison unrepresentable, wisdom 26).
And a named guidance/operating-pattern amendment (fable F8, §7). DEGRADE (Q2) is NOT in this
amendment set — it needs two unbuilt mechanisms (D2), deferred to PO follow-up cards.
...
- **I6 (specific refusal, and "none" ≠ "none qualifying").** Every deny names WHICH
  sub-condition failed and its sanctioned path — never the generic "needs reviewed-clean"
  that cost a team a day of misdiagnosis (PO att_df6af790). Specifically: when a
  reviewed-clean verdict PHYSICALLY EXISTS but fails to qualify under invariants 6/6b (not
  linked via `reviewsAssignmentId`, or author-not-R's-holder), the refusal MUST say so — it
  may NOT report "no reviewed-clean verdict," which is false and sends the reader hunting for
  a review that is sitting right there, mislinked (PO relay #2, live specimen asg_f0220808 /
  att_c277579a). Same class as remedy-named-before-deny (wisdom 4, 5).
...
- **Orchestrators (operating manual + orchestrator kernel):** under the shipped REQUIRE
  value, a producer dispatch must staff a reviewer on a DIFFERENT harness than the producer; a
  same-harness review no longer clears completion (and, until the deferred DEGRADE arm ships,
  a genuine single-harness window blocks completion — work-blocked, not a silent pass). This
  replaces the old third-party-opener dance (deleted by the opener-drop) with a
  cross-harness-reviewer expectation.
- **All openers:** non-producing cards (review/recon are auto-derived; orchestration/tracking/
  specimen/intake need it) are declared with `assign --non-producing` (Q1) so their holders
  can close them honestly; do NOT surrender delivered work or revoke-as-withdrawn to escape
  the rail.

The exact manual/kernel edit text lands with the code change (batched per wisdom 22); it is
named here as a required deliverable, not drafted, because the guidance edits should land the
day the commands ship (teach only what exists, wisdom 20).
