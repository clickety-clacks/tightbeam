...
parent notifications. Wake it and confirm its turn failed;
...
supports. Recorded live examples exist for claude on `claude-sonnet-5`, codex
on `gpt-5.6-sol`, and Pi on `opencode-go/gpt-5.6-luna`, each at effort
`medium`. Cursor is registered but has no recorded live model or live leg;
supply a model returned by that org's Cursor catalog rather than inventing a
default. Use a
