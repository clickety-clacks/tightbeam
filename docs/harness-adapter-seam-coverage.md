...
| `lib/tightbeam/gateway.ex` | credential service startup | no credential boot harvest | boot does not import legacy credentials; provider reads still use the exact home |
...
|---|---|---|
| `prepare_launch/3` | YES for claude, NO for codex | claude reads subscription credentials from its exact config home and injects only API keys through the kind-specific environment variable; codex reads its own `auth.json`, so its plan is kind-invariant — pinned by running both kinds through the conformance vectors rather than assumed |
