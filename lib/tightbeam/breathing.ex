defmodule Tightbeam.Breathing do
  @moduledoc "Query-time physical liveness; no liveness result is stored or published."
  ...
  defp session_result(txn, id) do
    case session(txn, id) do
      ...
      %{state: "retired"} = row ->
        result("session", id, false, "session_retired", %{session: evidence(row)})
  ...
  defp assignment_result(txn, id) do
    case assignment(txn, id) do
      ...
      %{state: "closed"} = row ->
        result("assignment", id, false, "assignment_closed", %{assignment: evidence(row)})

      row ->
        holder = session(txn, row.holder_key)
        base = %{assignment: evidence(row)} |> put_holder(holder)

        if match?(%{state: "retired"}, holder),
          do: result("assignment", id, false, "holder_retired", base),
          else:
            current_or_terminal(
              txn,
              "assignmentId = ?1",
  ...
  defp work_item_result(txn, id) do
    case work_item(txn, id) do
      ...
      %{state: state} = row when state != "open" ->
        result("work-item", id, false, "work_item_terminal", %{work_item: evidence(row)})

      row ->
        base = %{work_item: evidence(row)}
        ...
        case current_path(txn, "jobRef = ?1", [id], "work_item_id = ?1", [id]) do
          {:current, reason, current} ->
            result(
              ...
              true,
              reason,
              Map.put(base, current_key(reason), evidence(current))
            )
      ...
      _ ->
        result(
          ...
          Map.put(
            base,
            :open_assignments,
            Enum.map(answers, fn {row, answer} -> assignment_entry(row, answer) end)
          )
        )
    end
  end
  ...
  defp assignment_order(row, _), do: {1, 0, row.opened_at, row.id}
  ...
  defp current_or_terminal(txn, turn_where, turn_params, wake_where, wake_params, base, kind, id) do
    case current_path(txn, turn_where, turn_params, wake_where, wake_params) do
      {:current, reason, row} ->
        result(kind, id, true, reason, Map.put(base, current_key(reason), evidence(row)))

      :none ->
        case terminal_turn(txn, turn_where, turn_params) do
          %{status: status} = row when status in ["failed", "failed_unknown"] ->
            result(kind, id, false, "latest_terminal_failed", Map.put(base, :turn, evidence(row)))
          ...
          %{status: "canceled"} = row ->
            result(
              ...
              Map.put(base, :turn, evidence(row))
            )
  ...
  defp current_path(txn, turn_where, turn_params, wake_where, wake_params) do
    cond do
      row = newest_turn(txn, "running", turn_where, turn_params) ->
        {:current, "running_turn", row}

      row = newest_turn(txn, "queued", turn_where, turn_params) ->
        {:current, "queued_turn", row}

      row = pending_wake(txn, wake_where, wake_params) ->
        {:current, "pending_wake", row}
  ...
  defp put_holder(base, holder), do: Map.put(base, :holder_session, evidence(holder))
  ...
  defp evidence(row) do
    Map.new(row, fn {key, value} -> {camel(key), value} end)
  end

  defp camel(key) do
    value = key |> Atom.to_string() |> Macro.camelize()
