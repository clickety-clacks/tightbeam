defmodule Tightbeam.Breathing do
  ...
  defp session_in_txn(txn, id) do
    case Txn.q(txn, "SELECT sessionKey,state FROM sessions WHERE sessionKey=?1", [id]) do
      ...
      [[session_key, "retired"]] ->
        session = breathing_session([session_key, "retired"])
        result("session", id, false, "session_retired", %{session: session})

      [[session_key, state]] ->
        session = breathing_session([session_key, state])
  ...
  # Line-local public fields match main's canonical breathing serializers.
  defp breathing_session([session_key, state]), do: %{sessionKey: session_key, state: state}

  defp breathing_assignment([id, opened_at, state, outcome, holder_key, work_item_id]) do
    %{
      id: id,
      openedAt: opened_at,
      state: state,
  ...
  defp breathing_work_item([id, state, fail_reason]),
