defmodule Tightbeam.Firehose.Publisher do
  ...
  @transactional_verbs MapSet.new(
                         ~w(ask answer return work-item-create work-item-update work-item-icebox work-item-reopen work-item-close work-item-fail assign dispatch attest revoke-assignment reopen-assignment wake condition artifact-record read-marker-set read-marker-clear critical role-create role-bind role-rm spawn retire operator-ask operator-rule operator-withdraw rule effort-rule withdraw approve-device deny-device revoke-device add-user promote-user config host-env-set host-env-unset register-host identity-edit identity-relearn learn unlearn kungfu-scaffold)
                       )
  ...
  defp canonical_in_txn(txn, %{verb: verb} = call, result)
       when verb in ~w(assign dispatch revoke-assignment reopen-assignment) do
  ...
  def committed_in_txn(%Txn{} = txn, class, payload, refs \\ %{}) do
    if class == "kungfu.updated" do
      Txn.handoff(txn, Hub, {:publish, committed_notice(class, payload, refs)})
    else
      Txn.handoff(txn, Hub, {:committed, class, payload, refs})
    end
  end

  @doc "Capture one artifact lifecycle snapshot independently of dispatch flags."
  def artifact_in_txn(%Txn{} = txn, class, snapshot, call \\ %{}) do
    refs = %{
      "artifactId" => snapshot.artifact_id,
      "sessionKey" => snapshot.created_by_session,
      "workItemId" => snapshot.work_item_id
    }

    Txn.handoff(txn, hub(call), {:committed, class, snapshot, refs})
  end
  ...
  @doc false
  def wire_notice(%{"class" => "kungfu.updated", "payload" => payload} = notice) do
    allowed = ~w(class resource op refs payload type schemaVersion subscriptionId seq)

    unless Enum.all?(Map.keys(notice), &(&1 in allowed)),
      do: raise(ArgumentError, "invalid kungfu wire notice")

    canonical = committed_notice("kungfu.updated", payload, notice["refs"] || %{})

    for field <- ~w(resource op), Map.has_key?(notice, field) do
      unless notice[field] == canonical[field],
        do: raise(ArgumentError, "invalid kungfu wire notice")
    end

    Map.merge(canonical, Map.take(notice, ~w(type schemaVersion subscriptionId seq)))
  end

  def wire_notice(%{"class" => class} = notice) do
    if class in Registry.classes(),
      do: notice,
      else: raise(ArgumentError, "unsupported state notice")
  end

  def wire_notice(_), do: raise(ArgumentError, "unsupported state notice")

  def encode_wire_notice(%{"class" => "kungfu.updated"} = notice),
    do: notice |> wire_notice() |> encode_wire_notice(%{})

  def encode_wire_notice(notice), do: encode_wire_notice(notice, Tightbeam.ModelCatalog.get())

  @doc false
  def encode_wire_notice(%{"class" => class, "payload" => payload} = notice, catalog) do
    case Registry.fetch(class) do
      {:ok, %{resource: resource}} when is_map(payload) ->
        cond do
          StateResources.admin_resource?(resource) ->
  ...
  def committed_notice("kungfu.updated" = class, payload, refs)
      when is_map(payload) and is_map(refs) do
    name = payload["name"] || payload[:name]

    if not is_binary(name) or name == "" or
         (Map.has_key?(refs, "name") and refs["name"] != name) or
         Enum.any?(Map.keys(refs), &(&1 != "name")) do
      raise ArgumentError, "invalid kungfu notice references"
    end

    {:ok, row} = Registry.fetch(class)
    item = StateResources.kungfu(payload)
