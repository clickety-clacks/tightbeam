defmodule Tightbeam.Firehose.Publisher do
  ...
  @transactional_verbs MapSet.new(
                         ~w(ask answer return work-item-create work-item-update work-item-icebox work-item-reopen work-item-close work-item-fail assign dispatch attest revoke-assignment wake condition artifact-record read-marker-set read-marker-clear critical role-create role-bind role-rm spawn retire operator-ask operator-rule operator-withdraw rule effort-rule withdraw approve-device deny-device revoke-device add-user promote-user config host-env-set host-env-unset register-host identity-edit identity-relearn learn unlearn kungfu-scaffold)
                       )
  ...
  defp canonical_result(db, %{verb: verb} = call, result)
       when verb in ~w(assign dispatch revoke-assignment) do
