defmodule Tightbeam.Firehose.Rebuild do
  ...
  def fetch(_db, _class, _refs, user_id, _is_admin)
      when not is_binary(user_id) or user_id == "", do: :forbidden
  ...
  def fetch(db, class, refs, user_id, is_admin)
    ...
    with true <- class != "kungfu.updated" or StateVisibility.kungfu_visible?(is_admin),
         {:ok, %{rebuild: true}} <- Registry.fetch(class),
  ...
  defp query(db, class, refs, _user_id)
       when class in ~w(user.added user.promoted user.demoted),
  ...
  defp query(db, "identity.updated", refs, _user_id) do
    StateResources.query_identity(db, fetch!(refs, "name"))
  end
