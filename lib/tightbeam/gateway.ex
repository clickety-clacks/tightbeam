defmodule Tightbeam.Gateway do
  ...
  defp harness_binary_readiness(cli_bin) do
    ...
    cond do
      ...
      Enum.all?(results, fn {_harness, result} -> harness_cli_absent?(result) end) ->
        ...
        {:error,
         {:no_harness_cli,
          "Tightbeam cannot start because no usable harness CLI is installed. " <>
  ...
  defp turn_runner(config) do
    ...
    fn turn ->
      ...
      outcome =
        with {:ok, adapter, generation} <- traced_checkout(db, turn, session, config),
             {:ok, harness_session} <-
  ...
  defp traced_checkout(db, turn, session, config) do
    with :ok <- trace_stage_started(db, turn, :checkout) do
      case traced_call(db, turn, :checkout, fn -> checkout_adapter(session, config) end) do
