defmodule Tightbeam.GithubAuth do
  @moduledoc """
  The gateway-side health adapter for the GitHub credential kind: where the provider-owned
  ...
  @doc "The default profile home on this registered machine."
  @spec config_dir(String.t()) :: String.t()
  def config_dir(base_dir),
    do:
      Tightbeam.GithubCredentials.home(base_dir, Tightbeam.Placement.local_host_name(), "default")

  @doc false
  def config_dir(base_dir, machine, profile),
    do: Tightbeam.GithubCredentials.home(base_dir, machine, profile)
  ...
  def probe(base_dir, hostname, remote_url) do
    ...
    with :valid <- Tightbeam.GithubCredentials.storage(config_dir(base_dir)),
      ...
      :absent ->
        {:error, :needs_onboarding, "the elected GitHub credential home is absent"}
  ...
  def scrub_detail(detail) do
    ...
    |> String.replace(~r{([a-zA-Z][a-zA-Z0-9+.-]*://)[^/@\s]+@}, "\\1[redacted]@")
    |> String.trim()
  end
  ...
  def classify_api_failure(detail) do
    ...
    cond do
      Regex.match?(~r/^http\/\S+\s+401\b/m, down) ->
        :expired

      Regex.match?(~r/^http\/\S+\s+403\b/m, down) ->
        :insufficient_scope
