defmodule Tightbeam.Harness.Claude do
  ...
  @adapter_replacements [
    {
      ...
      "    async unstable_forkSession(params) {\n        if (this.providerUpdate)\n            await this.providerUpdate;\n        const forked = await forkSession(params, {\n            liveMessageIdToUuid: this.sessions[params.sessionId]?.messageIdToUuid,\n            messageIdForGrouping,\n        });\n        // A disk-only fork must become resident before configuration or prompts.\n        const resumed = await this.resumeSession({ ...params, sessionId: forked.sessionId });\n        return { ...resumed, sessionId: forked.sessionId };\n    }\n"
    },
    {
  ...
  # Observed native session keys and transient configuration locks. Projection
  # does not own these paths; arbitrary files under sessions/ still fail census.
  @native_home_patterns [
    ~r"\Asessions/\d+\.[0-9a-f]{64}\.key\z",
    ~r"\Asessions/\d+\.json\z",
    ~r"\A\.claude\.json\.lock\z"
  ]
  ...
  def conformance_vectors do
    ...
    Support.conformance_vectors(__MODULE__, %{
      native_home_paths: ["sessions/2192740.json", "unexpected.txt"],
