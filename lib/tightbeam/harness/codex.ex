defmodule Tightbeam.Harness.Codex do
  ...
  # Mike ruling dr_4409f63c authorizes the remaining temporary compatibility
  # behavior on codex-acp 1.9.0. These three replacements are one logical
  # identity patch across new, resume, and load. Account updates are upstream in
  # 1.9.0 as the connection-level `_auth/status_update` carrier, and native
  # subagent lifecycle support is also upstream; neither may return to this list.
  ...
  def classify_auth_event(%{"authStatus" => %{"kind" => kind}})
      when kind in ["account", "api_key", "external", "gateway"],
      do: :transient
  ...
  def conformance_vectors do
    ...
    Support.conformance_vectors(__MODULE__, %{
      ...
      auth_events: [
        %{
          case: "positive",
          envelope: %{"authStatus" => %{"kind" => "none", "label" => "Not logged in"}},
          expected: :terminal
        },
