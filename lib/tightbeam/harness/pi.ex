defmodule Tightbeam.Harness.Pi do
  ...
  def prepare_launch(target, home, opts) do
    ...
    probe =
      if Keyword.fetch!(opts, :statutes) do
        ...
        [probe_cwd: probe_cwd, probe_model: gate_probe_model!(target)]
      else
        []
      end
  ...
  @doc false
  def gate_probe_model!(target) do
    state =
      target
      |> Map.put(:base_dir, target.host_config.base_dir)
      |> Map.put_new(:options, %{sh: target.sh})

    case PiProvider.fetch_pi_catalog(state) do
      {:ok, [_ | _] = entries} ->
        select_gate_probe_model(entries)

      {:error, reason} ->
        raise ArgumentError, "no runnable Pi model for the permission probe: #{inspect(reason)}"
    end
  end

  @doc false
  def select_gate_probe_model([]),
    do: raise(ArgumentError, "no runnable Pi model for the permission probe")

  def select_gate_probe_model([_ | _] = entries) do
    entry = Enum.find(entries, &gate_local_spark?/1) || Enum.find(entries, &gate_default?/1)

    case entry do
      %{family: family} -> Model.new(family)
      nil -> raise ArgumentError, "no runnable Pi model for the permission probe"
    end
  end

  defp gate_local_spark?(%{
         family: "spark/" <> _,
  ...
  defp gate_local_spark?(_entry), do: false

  defp gate_default?(%{family: family}) when family == @probe_model.family, do: true
  defp gate_default?(_entry), do: false
  ...
  def owned_home_entries do
    Support.owned_home_entries("extensions/tightbeam.ts", [@models_json])
  end
