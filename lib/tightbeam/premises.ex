defmodule Tightbeam.Premises do
  ...
  @doc """
  Unlink a mistaken park premise.

  Claims and checks are immutable, but a LINK is a routing decision, and every
  state owes an agent-reachable repair verb (philosophy gate 3). Unlinking
  destroys no observation: the claim and its whole check lineage remain.
  """
  @spec unlink_park(DB.server(), String.t(), String.t(), String.t()) :: :ok
  def unlink_park(db, park_kind, park_id, claim_id) do
    {:ok, _} =
      DB.query(
        db,
        "DELETE FROM park_premises WHERE parkKind = ?1 AND parkId = ?2 AND claimId = ?3",
        [park_kind, park_id, claim_id]
      )
