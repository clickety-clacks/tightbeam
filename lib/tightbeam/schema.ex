defmodule Tightbeam.Schema do
    ...
    case DB.foreign_key_rebuild(db, fn txn ->
           previous_shape =
             case Txn.q(txn, "SELECT shape FROM schema_stamp") do
               [[shape]] when shape in [@previous_shape, @legacy_previous_shape] ->
                 shape

               rows ->
                 incompatible_cold_start!(
                   "legacy_witness_missing",
                   "predecessor #{inspect(rows)}"
                 )
             end
           ...
           case Txn.q(txn, "PRAGMA foreign_key_check") do
             [] -> :ok
             rows -> incompatible_cold_start!("legacy_witness_missing", inspect(rows))
           end
  ...
  defp check_shape(db) do
    case DB.query(db, "SELECT shape FROM schema_stamp") do
      ...
      {:ok, [[found]]} ->
        ...
        This build can migrate only #{@model_identity_shape} through #{@previous_shape},
        or #{@previous_shape} / #{@legacy_previous_shape} directly, to #{@shape}.
    ...
    try do
      case DB.transaction(db, &migrate_model_identity_v1_in_txn/1) do
        ...
        {:error, error} ->
          raise ShapeError,
            message:
              "migration #{@model_identity_shape} -> #{@previous_shape} failed and was rolled back: #{Exception.message(error)}"
      end
    after
  ...
  defp migrate_model_identity_v1_in_txn(%Txn{} = txn) do
    ...
    if Txn.changes(txn) != decision_request_count do
      raise ShapeError,
        message:
          "migration #{@model_identity_shape} -> #{@previous_shape} copied #{Txn.changes(txn)} of #{decision_request_count} decision requests"
    end
    ...
    if Txn.changes(txn) != message_count do
      raise ShapeError,
        message:
          "migration #{@model_identity_shape} -> #{@previous_shape} copied #{Txn.changes(txn)} of #{message_count} messages"
    end
    ...
    case Txn.q(txn, "PRAGMA foreign_key_check") do
      ...
      rows ->
        raise ShapeError,
          message:
            "migration #{@model_identity_shape} -> #{@previous_shape} left invalid foreign keys: #{inspect(rows)}"
    end

    Txn.q(
      txn,
      "UPDATE schema_stamp SET shape = ?1, stampedAt = ?2 WHERE shape = ?3",
      [@previous_shape, System.system_time(:millisecond), @model_identity_shape]
    )

    if Txn.changes(txn) != 1 do
      raise ShapeError,
        message:
          "migration #{@model_identity_shape} -> #{@previous_shape} lost its exact stamp transition"
    end
