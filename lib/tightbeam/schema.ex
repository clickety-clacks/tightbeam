defmodule Tightbeam.Schema do
  ...
  @cursor_provider_previous_shape @pi_shape
  ...
  @cursor_provider_values "'anthropic','openai','cursor','opencode_go','local_openai'" <>
                            if(
  ...
  def ensure_supervision_liveness_v1_in_txn(%Txn{} = txn, activated_at, opts)
    ...
    enforcement_objects =
      if shape in [@reparent_shape, @durability_shape, @pi_shape, @cursor_provider_shape],
  ...
  defp migrate_cursor_provider_v1_020(db) do
    ...
    :ok = DB.execute(db, "PRAGMA foreign_keys = OFF")
    # The current schema has triggers on other tables that reference sessions.
    # Preserve their source-owned references during the table swap.
    :ok = DB.execute(db, "PRAGMA legacy_alter_table = ON")
