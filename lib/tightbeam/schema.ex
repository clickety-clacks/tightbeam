defmodule Tightbeam.Schema do
  ...
  # column. The v4 stamp upgrades to v5 before the holder-role migration runs.
  # The holder-role migration changes no table shape: its new stamp is the
  # one-time receipt that prevents later role rebinds from rewriting ambiguous
  # historical NULLs on each boot.
  @shape "coordination-fabric-v1-phase1-v5-holder-role-v1"
  ...
  never to deduce a shape and accommodate it. The two accepted predecessors
  are named explicitly; their operational-parent and holder-role upgrades are
  exact, ordered, and transactional. No shape is inferred from stored DDL.
  """
  ...
  @doc false
  @spec upgrade_holder_role_v1(DB.server(), keyword()) :: :ok
  def upgrade_holder_role_v1(db, opts \\ []) when is_list(opts) do
    case DB.transaction(db, fn txn ->
           case Txn.q(txn, "SELECT shape FROM schema_stamp") do
             [[@previous_shape]] ->
               :ok
             ...
             rows ->
               raise ShapeError,
                 message: "incompatible_holder_role_v1: predecessor stamp #{inspect(rows)}"
           end

           _affected_rows = Tightbeam.Assignments.backfill_holder_roles_in_txn(txn)

           if Keyword.get(opts, :fail_after_backfill),
             do: raise("forced holder-role migration interruption")
           ...
           if Txn.changes(txn) != 1,
             do: raise(ShapeError, message: "incompatible_holder_role_v1: stamp race")
      ...
      {:error, error} ->
        raise ShapeError,
          message: "incompatible_holder_role_v1: upgrade failed: #{Exception.message(error)}"
    end
  end
