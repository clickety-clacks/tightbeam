defmodule Tightbeam.StaleTurnSettlement do
  ...
  defp correlation(db, request, adapter_gen) do
    with {:ok, turn} <- turn(db, request.session_key, request.turn_seq),
         ...
         {:ok, acp_request_id} <- dispatch_request(db, request.turn_seq, adapter_gen),
  ...
  defp commit(db, request, correlation, valid?) do
    case DB.transaction_then(
           db,
           fn txn -> commit_in_txn(txn, request, correlation, valid?) end,
           fn txn, result ->
      ...
      {:ok, {:ok, result}} ->
        {publication, result} = Map.pop(result, :route_publication)
        if publication, do: Tightbeam.EventLog.publish(publication.plan)
        {:ok, result}
  ...
  defp do_commit_in_txn(txn, request, correlation, valid?) do
    with :ok <- active_in_txn(txn, request.session_key),
         ...
         :ok <- running(turn),
         {:ok, acp_request_id} <-
           dispatch_request_in_txn(txn, request.turn_seq, correlation.adapter_gen),
         %{id: pointer_id, harness_session_id: harness_session_id} <-
      ...
      transition =
        Ledger.turn_terminal_transition_in_txn(txn, request.turn_seq, "running", status)
      ...
      if Txn.changes(txn) == 1 do
        ...
        put_replay_in_txn(txn, request, result)
        publication = HarnessHealth.settle_other_route_in_txn(txn, request.turn_seq, status, now)
        require_fence!(valid?)
        {:ok, Map.put(result, :route_publication, publication)}
      else
  ...
  defp dispatch_request(db, turn_seq, generation) do
    case DB.query(
           db,
           "SELECT acpRequestId,adapterGen FROM turn_lifecycle_events WHERE turnSeq=?1 AND kind='prompt_dispatched'",
           [turn_seq]
         ) do
      {:ok, [[request_id, ^generation]]} when is_integer(request_id) and request_id > 0 ->
        {:ok, request_id}
  ...
  defp dispatch_request_in_txn(txn, turn_seq, generation) do
    case Txn.q(
           txn,
           "SELECT acpRequestId,adapterGen FROM turn_lifecycle_events WHERE turnSeq=?1 AND kind='prompt_dispatched'",
           [turn_seq]
         ) do
      [[request_id, ^generation]] when is_integer(request_id) and request_id > 0 ->
        {:ok, request_id}
