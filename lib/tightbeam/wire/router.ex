defmodule Tightbeam.Wire.Router do
  ...
  @agent_verbs ~w(wake condition facts-read artifact-record artifact-get artifacts spawn retire critical adjudicate inspect cancel tune approve-device deny-device revoke-device promote-user config register-host identity-edit identity-status identity-relearn identity-apply kungfu-scaffold onboard role-create role-bind role-rm role-list assign dispatch assignment-get attest attests revoke-assignment assignments work-item-create work-item-get work-item-list work-item-update run-tests run-smoke cancel-producer-job rule waive revoke-waiver withdraw decision-requests decision-request)
