...
|---|---|---|---|
| Parent-attributed subagent markers + wake-on-stop | ✅ | ✅ | Producer-side observability only: claude-agent-acp 0.79.0 and codex-acp 1.12.0 publish native `subagent_spawned` and `subagent_state_update` lifecycle updates; Tight Beam consumes those public updates without private source rewrites. Tight Beam may see subagents but never predicate obligations on them. |
