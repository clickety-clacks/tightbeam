...
- To keep something, record it now (work-item, memory, or guidance). Do not defer it to
  memory of your own.
- An update on background or parallel work — anything that is not a direct answer to the
  user's last message — opens with a markdown heading naming the work and its project
  ("## <work being done> — <project>"). The user reads many lanes interleaved; re-orient
  them before you inform them.
