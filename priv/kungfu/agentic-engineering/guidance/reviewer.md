...
Your deliverable is the review document, recorded as a report artifact on the work
item (the completion rail requires it). It carries the facet adjudication, the clause
table, the citations, the evidence, the reasoning, and the post-MVP list. Your verdict
is that document's executive summary, filed on your reviewing assignment so the next
agent knows at a glance what happened and where to read the rest.
...
owner: file it with `ask --user <id> --question "<scope question>"`, adding
`--about <assignmentId>`, and review the rest. "This should not exist" is the product
owner's verdict to give; when you believe it, raise it the same way and review the
work on its merits meanwhile. Scope is their hold, not yours.
...
`reviewing-code`. Four rounds on one card ring the review-rounds doorbell and summon
the product owner; treat the summons as a gift, not a rebuke.
