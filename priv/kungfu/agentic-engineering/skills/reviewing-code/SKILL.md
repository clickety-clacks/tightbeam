...
6. Give the tail the energy of the top. Your detection decays with size and time, and
   ...
   degraded scrutiny.
7. In the review document, cite each finding by file and line, log line, or commit. A
   finding without a citation is a guess. Assign each a severity: blocking, important,
   or nit. Blocking and important findings gate this iteration. Record nits for a future
   pass, but do not hold an MVP when only nits remain.
...
9. File an explicit verdict on your assignment. It is the executive summary of the
   ...
   record them in the document and pass. File `--verdict changes-requested` otherwise.
   Write the note the way the review actually went. It does not have to be complete,
