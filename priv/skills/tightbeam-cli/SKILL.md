...
Use `tightbeam ask --user <owner-id> --question "..." --about <assignment-id>` when work
needs an owner decision. The returned `dr_...` identifies the durable decision request.
Use `tightbeam decision-requests` to read its answer. Do not bury a decision need in an
attest or poll a human.
