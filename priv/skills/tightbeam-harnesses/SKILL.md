...
description: Per-harness capability support matrix (Claude, Codex, Cursor, and Pi) — what works where and by what mechanism. Consult before promising or relying on a harness-specific capability.
---
...
- CAP-001 sessions/turns/cancel/load: Claude, Codex, and Pi PARITY; Cursor has
  a local ACP contract.
...
- CAP-003 slash commands: PARITY passthrough. Claude
  ...
  and domain commands. Pi exposes `/compact /autocompact /export /session
  /name /steering /follow-up /changelog`, plus its discovered commands and
  skills. Cursor has local ACP passthrough, but its vendor vocabulary is not
  enumerated or promised.
- CAP-004 projected identity: keep Claude system-prompt append metadata and
  ...
  first-request delivery. Pi uses its Tightbeam-owned global
  `before_agent_start` extension reading the reserved per-cwd identity carrier.
  Cursor has local harness instruction metadata.
- CAP-005 native skills: PARITY progressive disclosure under `.claude/skills`,
  `.codex/skills`, and `.pi/skills`. Cursor materializes local skills under
  `.cursor/skills`. Tightbeam owns `tightbeam__*` only.
- CAP-006 vendor-native skills/commands: preserved additively. Pi is PARITY.
  Cursor local reconciliation sentinels prove leaves outside its owned set survive.
- CAP-007 gate statutes: Claude uses PreToolUse settings and Codex uses
  PreToolUse hooks after the fail-closed boot wiring-check. Pi's Tightbeam-owned
  global `tool_call` extension runs the same compiled command hooks before
  execution and passes the fail-closed wiring check. Cursor compiles faithful
...
- CAP-009 harness-home credential ownership: use one regular credential file
  per exact machine/harness home for Claude, Codex, and Pi. Reconciliation must not
...
- CAP-010 token environment: Claude, Codex, and Pi retain their current mechanisms.
  Cursor locally injects only `CURSOR_API_KEY`. No subscription-longevity
  equivalence is claimed.
- CAP-011 onboarding: Claude, Codex, and Pi use `tightbeam onboard <provider>`.
  Cursor is API-key-only `DIV-CURSOR-API-KEY-ONLY`.
  Subscription vectors are explicitly unsupported, fail closed, and inject no key.
- CAP-012 progress: Claude, Codex, and Pi provide rich ACP updates. Cursor has a local
  ACP contract; no live Cursor run is claimed.
- CAP-013 usage telemetry: Claude and Codex emit usage. Pi has
  `DIV-USAGE-PI-UNPROJECTED` because pi-acp exposes session stats only through
  `/session`, not Tightbeam's usage event channel. Cursor has a local ACP
  contract; no live Cursor run is claimed.
- CAP-014 compaction: Claude `DIV-COMPACTION-CLAUDE-ABSENT`; Codex
  `DIV-COMPACTION-CODEX-UNPROJECTED`; Pi supports `/compact` and `/autocompact`
  but Tightbeam does not project their structured lifecycle, so
  `DIV-COMPACTION-PI-UNPROJECTED`; Cursor
  `DIV-COMPACTION-CURSOR-UNPROJECTED`.
- CAP-015 hash-gated homes: Claude, Codex, and Pi preserve their owned entries.
  Cursor has a local contract and owns only `cli-config.json` and compiled `hooks.json`.
- CAP-016 harness switching: Claude, Codex, and Pi retain the history barrier;
...
- CAP-017 auth-event classification: Claude `DIV-AUTH-CLAUDE-UNKNOWN`; Codex
  classifies account updates; Pi has `DIV-PI-ACP-NO-AUTH-EVENT` because pi-acp
  emits no account event. Cursor is negative-tested as always unknown under
  `DIV-AUTH-CURSOR-UNSUPPORTED`.
- CAP-018 credential liveness: Claude, Codex, and Pi use bounded authenticated probes.
  Claude calls `GET /v1/models?limit=1`; Codex calls
  `GET /backend-api/wham/accounts/check`; Pi makes a minimum-size
  `gpt-5.6-luna` Responses request with the banked OpenCode Go key. `:live`
  passes, `:dead` fails, and
