defmodule Tightbeam.AdminProjectionTest do
  ...
  alias Tightbeam.Firehose.{Hub, Registry, Publisher}

  setup context do
    if context[:admin_fixture] do
      base_dir =
  ...
  test "A1 maps every ruled class both ways and A6 uses the exact shared item bytes", ctx do
    ...
    cases = [
      ...
      %{
        ...
        resource_classes: ["user.added", "user.demoted", "user.promoted"]
      },
      %{
    ...
    Enum.each(cases, fn expected ->
      ...
      shape_error =
        if expected.resource == "kungfu",
          do: ~r/^invalid kungfu projection shape$/,
          else: ~r/extra or missing/

      assert_raise ArgumentError, shape_error, fn ->
  ...
  test "a failed Git publication does not advance a stamp or emit state", ctx do
    ...
    assert %{code: "identity_publication_non_fast_forward"} =
             handler.(
  ...
  @tag admin_fixture: true
  test "kungfu serves exactly three raw document paths with exact hashes and no credential traversal",
       ctx do
    dir = Path.join(ctx.base_dir, "identity")
    bundle = Path.join(dir, "kungfu/raw-proof")
    File.mkdir_p!(bundle)

    File.write!(
      Path.join(bundle, "manifest.toml"),
      "purpose = \"Raw byte proof.\"\nphrases = [\"z\", \"a\"]\nroot_archetype = \"default\"\n"
    )

    docs = [
      {"README.md", "  token = literal-synthetic\r\n/home/synthetic/.ssh/key\nλ\n\n"},
      {"capabilities.md", "authorization: Bearer synthetic-only\n"},
      {"preferred-models.md", "models\tunchanged\n"}
    ]

    for {path, bytes} <- docs, do: File.write!(Path.join(bundle, path), bytes)
    File.write!(Path.join(bundle, "intake.md"), "excluded-intake")
    File.write!(Path.join(bundle, ".hidden"), "excluded-hidden")
    # A cyclic synthetic sentinel fails any attempted credential traversal.
    bank = Path.join(ctx.base_dir, "credentials")
    File.ln_s!("credentials", bank)
    git!(dir, ["add", "kungfu/raw-proof"])
    ...
    git!(dir, [
      ...
      "commit",
      "-m",
      "synthetic raw document fixture"
    ])

    revision = git!(dir, ["rev-parse", "main"])
    git!(dir, ["update-ref", "refs/heads/tightbeam/live", revision])
    item = Identity.public_kungfu(ctx.base_dir, "raw-proof")
    assert item["installedRevision"] == revision
    assert item["phrases"] == ["a", "z"]

    assert item["documents"] ==
             Enum.map(docs, fn {path, bytes} ->
               %{
                 "path" => path,
                 "content" => bytes,
    ...
    assert {:ok, "credentials"} = File.read_link(bank)
    shared = StateResources.kungfu(item)
    bytes = StateResources.encode_admin_item("kungfu", shared)
    notice = Publisher.committed_notice("kungfu.updated", item, %{"name" => "raw-proof"})
    wire = Publisher.encode_wire_notice(notice)
    assert wire =~ ~s("payload":#{bytes})
    assert JSON.decode!(wire)["payload"]["documents"] == item["documents"]
    refute wire =~ "excluded-intake"
    refute wire =~ "excluded-hidden"

    assert_raise ArgumentError, ~r/invalid kungfu name/, fn ->
      Identity.public_kungfu(ctx.base_dir, "../raw-proof")
    end

    assert_raise ArgumentError, ~r/invalid kungfu notice references/, fn ->
      Publisher.committed_notice("kungfu.updated", item, %{"name" => "other"})
    end

    invalid = put_in(item, ["documents", Access.at(0), "sha256"], String.duplicate("0", 64))
    assert_raise ArgumentError, fn -> StateResources.kungfu(invalid) end
    assert :ok = Org.put_setting(ctx.db, "private-token", "synthetic-config-value")

    assert StateResources.config(StateResources.query_config(ctx.db, "private-token"))["value"] ==
             nil
    ...
    assert {:ok, %{changed: true}} =
             Placement.register_host_with_firehose(
               ...
               %{
                 ssh: "synthetic@host",
                 base_dir: "/synthetic/private",
                 cli_bin: "/synthetic/bin"
               },
               firehose_call("register-host", %{})
             )
    ...
    host_bytes =
      ctx.db
      |> StateResources.query_host("raw-proof-host")
      |> StateResources.host()
      |> JSON.encode!()
  ...
  @tag :tmp_dir
  test "version floors serialize concurrent commits, roll back, and survive guarded restart", %{
    tmp_dir: tmp
  } do
    Tightbeam.GuardRuntimeFixture.run!(
      tmp,
      "firehose_admin_restart.exs",
      "guarded-admin-floor-restart: ok"
    )
  end
