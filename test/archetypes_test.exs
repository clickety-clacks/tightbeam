defmodule Tightbeam.ArchetypesTest do
    ...
    for relative_command <- [
      ...
      assert {_denial, 2} =
               System.cmd(
                 ...
                 env: [{"TB_RETIRE_GATE_INPUT", relative_call}],
                 stderr_to_stdout: true
               )
    end

    quoted_assignment_call =
      JSON.encode!(%{
        "tool_name" => "Bash",
        "tool_input" => %{"command" => ~s(TB_NOTE="a b" tightbeam retire --session child)}
      })

    assert {_denial, 2} =
             System.cmd("sh", ["-c", ~s(printf '%s' "$TB_RETIRE_GATE_INPUT" | ) <> command],
               env: [{"TB_RETIRE_GATE_INPUT", quoted_assignment_call}],
               stderr_to_stdout: true
             )

    mention_call =
      ~s({"tool_name":"Bash","tool_input":{"command":"echo tightbeam retire --session child"}})
  ...
  test "unknown keys, bad harness defaults, and missing reference locations fail boot", ctx do
    ...
    cases = [
      ...
      {"name = \"bad\"\n[defaults]\nharness = \"other\"\n",
       ~r/unknown harness "other"; expected one of: claude, codex, cursor, fixture/},
      {"name = \"bad\"\n[references]\nrepo = { access = \"read\" }\n",
