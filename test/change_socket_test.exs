defmodule Tightbeam.Wire.ChangeSocketTest do
  ...
  test "heartbeat carries the latest connection sequence", ctx do
    ...
    Hub.publish(ctx.hub, notice(ctx, "wi_trailing"))
  ...
  test "publication before the registration cut is never delivered", ctx do
    ...
    Hub.publish(ctx.hub, notice(ctx, "wi_before"))
    ...
    Hub.publish(ctx.hub, notice(ctx, "wi_after"))
  ...
  test "the per-connection queue is exact and overflow closes once", ctx do
    ...
    Enum.each(1..1_500, fn index -> Hub.publish(ctx.hub, notice(ctx, "wi_#{index}")) end)
  ...
  test "registry rows are both-way unique and cover effects from the real gateway table", ctx do
    ...
    expected_admin_rows = %{
      ...
      "user.demoted" => {"users", ["userId"], :query_user, :user, :user_visible?}
    }
  ...
  defp assert_registry_match!(handler_effects, rows) do
    # Archive/release are lifecycle entry points, not executable Gateway verbs.
    # ArtifactsTest drives their real commits and verifies the emitted classes.
    {:module, Tightbeam.Artifacts} = Code.ensure_loaded(Tightbeam.Artifacts)
    assert function_exported?(Tightbeam.Artifacts, :archive_session, 4)
    assert function_exported?(Tightbeam.Artifacts, :release, 2)

    declared = Enum.sort(Gateway.emitted_state_classes(handler_effects))
  ...
  defp notice(ctx, id) do
    {:ok, _} =
      DB.query(
        ctx.db,
        """
        INSERT INTO work_items(id,title,ownerUserId,state,createdByUser,createdAt)
        VALUES (?1,'Socket fixture','flynn','open','flynn',1)
        """,
        [id]
      )

    payload =
      StateResources.query_work_item(ctx.db, id, %{principal: {:user, "flynn"}})
      |> StateResources.work_item()
