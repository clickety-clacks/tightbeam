defmodule Tightbeam.CheckTierTest do
  ...
  test "attest-v1 database rebuild is FK-safe and idempotent" do
    ...
    for module <- [Tightbeam.CausalEvents, Devices, Org, WorkItems],
        do: :ok = module.ensure_schema(db)

    {:ok, _} =
      DB.query(db, "INSERT INTO users (userId, isAdmin, createdAt) VALUES ('flynn', 1, 1)")

    session(db, "holder", "flynn", "coder")

    :ok =
      DB.execute(db, """
      CREATE TABLE assignments (
        id TEXT PRIMARY KEY, subject TEXT NOT NULL, holderKey TEXT NOT NULL REFERENCES sessions(sessionKey),
        ...
        closedByUser TEXT NULL, closedBySession TEXT NULL,
    ...
    :ok =
      ...
      CREATE TABLE attests (
        ...
        note TEXT NULL, bySession TEXT NOT NULL REFERENCES sessions(sessionKey), ts INTEGER NOT NULL
      )
      """)
    ...
    {:ok, _} =
      DB.query(
        db,
        "INSERT INTO attests (id, assignmentId, kind, bySession, ts) VALUES ('att_close', 'asg_old', 'completion', 'holder', 2)"
      )

    {:ok, _} =
      DB.query(
        db,
        "UPDATE assignments SET state='closed', outcome='completed', closedAt=2, closedBySession='holder', closingAttestId='att_close' WHERE id='asg_old'"
      )
    ...
    assert [%{id: "asg_old", closingAttestId: "att_close"}] =
             Assignments.list(db, %{state: "all"})

    assert [%{id: "att_close", verdictKind: nil, byUser: nil}] =
             Assignments.list_attests(db, "asg_old")

    assert {:ok, _} =
             DB.query(
               db,
               "INSERT INTO assignments (id, subject, holderKey, openedByUser, openedAt) VALUES ('asg_new', 'new', 'holder', 'flynn', 3)"
             )
  end
