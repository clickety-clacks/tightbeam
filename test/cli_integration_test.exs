defmodule Tightbeam.CliIntegrationTest do
  ...
  test "immutable previous Gateway rolls back on one database and the candidate re-upgrades it",
    ...
    assert {:ok, [[^old_work_item_id, "I68 legacy telemetry", "i68-flynn"]]} =
             DB.query(
               candidate_db,
               "SELECT id, title, ownerUserId FROM work_items WHERE id = ?1",
               [old_work_item_id]
             )
    ...
    assert :ok = Tightbeam.Schema.ensure_all(candidate_db)
    ...
    candidate_opts =
      Router.init(
        ...
        cursor_signing: cursor_signing!(rollback_base),
