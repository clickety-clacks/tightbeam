defmodule Tightbeam.CliIntegrationTest do
  ...
  setup do
    ...
    # No rebuild here either. This file used to refresh the binary itself as a
    # defence against rail_script_test rebuilding it mid-suite; that defence was
    # a second mutation of the same shared artifact, which merely moved the race
    # rather than ending it. Both are now REFUSALS instead: rail_script_test
    # (att_9e4e06df) refuses a stale binary and names the build command, and the
    # gate already requires that build before it runs. Nothing rewrites this
    # path while the suite reads it.
