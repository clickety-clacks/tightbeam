defmodule Tightbeam.DispatchTest do
  ...
  test "supervision dispatch preserves ordinary return shapes", %{db: db} do
    call = %{
      verb: "wake",
      origin: "process:tightbeam",
      principal: {:process, "tightbeam"},
    ...
    handlers = %{"wake" => fn _ -> %{delivered: true} end}
    expected = Dispatch.dispatch(db, handlers, call)

    assert expected ==
             Dispatch.dispatch_with_policy_denial_transition(db, handlers, call, %{
               kind: "policy_denied",
               evaluation_clock: 5_000
             })
