defmodule Tightbeam.GithubAuthTest do
  ...
  test "config_dir resolves the provider-owned default profile home" do
    assert GithubAuth.config_dir("/base") == "/base/credential-homes/testhost/github/default"
  end

  test "env pins GH_CONFIG_DIR unconditionally" do
    assert GithubAuth.env("/base") == [
             {"GH_CONFIG_DIR", "/base/credential-homes/testhost/github/default"}
           ]
  end
  ...
  test "classify_api_failure matches the Rust classifier's contract" do
    assert GithubAuth.classify_api_failure("invalid oauth token") == :unknown
    assert GithubAuth.classify_api_failure("You are not logged into any accounts") == :unknown
    assert GithubAuth.classify_api_failure("HTTP/2 401 unauthorized") == :expired
    assert GithubAuth.classify_api_failure("missing required scope") == :unknown
