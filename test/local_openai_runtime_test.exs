defmodule Tightbeam.LocalOpenAiRuntimeTest do
      ...
      options: %{
        ...
        sh: fn command ->
          ...
          cond do
            ...
            String.contains?(joined, "/bin/test") ->
              {"", 0}
  ...
  test "remote provider read refuses when the redaction parser is unavailable" do
    secret = "TB_REMOTE_REDACTION_NO_PARSER_SECRET"
      ...
      options: %{
        ...
        sh: fn command ->
          ...
          cond do
            ...
            String.contains?(joined, "pi.dev/api/models/providers/opencode-go") ->
            ...
            String.contains?(joined, "JSON.parse") ->
              {"", 65}

            true ->
              {~s({"name":"spark","type":"local-openai","endpoint":"https://spark.example/v1","apiKey":"#{secret}"}) <>
    ...
    result = Providers.read_all_target(state)
    assert {:error, {:remote_provider_read_failed, "spark", {:exit, 65}}} = result
    refute inspect(result) =~ secret
  end

  test "remote provider read refuses malformed redaction output without returning its bytes" do
    owner = self()
    secret = "TB_REMOTE_REDACTION_MALFORMED_SECRET"
    descriptor = ~s({"name":"spark","apiKey":"#{secret}"})
      ...
      options: %{
        ...
        sh: fn command ->
          ...
          cond do
            ...
            String.contains?(joined, "JSON.parse") ->
              output = "SyntaxError: malformed provider descriptor at position 18\n"

              send(owner, {:redactor_output, output, command})
              {output, 65}
    ...
    result = Providers.read_all_target(state)
    assert {:error, {:remote_provider_read_failed, "spark", _reason}} = result
    assert_receive {:redactor_output, output, command}
