defmodule Tightbeam.PremisesTest do
  ...
  test "a mistaken link is repairable by an agent without destroying an observation",
       %{db: db} do
    claim = declare(db)
    {:ok, check} = record(db, claim, result: "refuted", observed_source_version: "sha-a")
    link(db, "patrol_hold", "patrol_1", [claim])

    assert {:refused, :refuted, _} =
             Premises.park_admission(db, "patrol_hold", "patrol_1", now: @now)

    # A link is a routing decision, so it is the one part of this shape an
    # agent may undo. UPDATE is still refused: repair is unlink and relink.
    assert {:error, %DB.Error{message: message}} =
             DB.query(db, "UPDATE park_premises SET claimId=?1 WHERE parkId='patrol_1'", [claim])

    assert message =~ "park_premises_immutable"

    assert :ok = Premises.unlink_park(db, "patrol_hold", "patrol_1", claim)

    assert {:ok, :legacy_untyped} =
             Premises.park_admission(db, "patrol_hold", "patrol_1", now: @now)

    assert {:ok, [["refuted"]]} =
             DB.query(db, "SELECT result FROM premise_checks WHERE checkId=?1", [check])

    assert {:refuted, _} = Premises.premise_state(db, claim, @now)
  end
