defmodule Tightbeam.ReleaseCandidateWorkflowTest do
  ...
  test "preparation refuses to run without a fetched origin/0.1.x" do
    fixture = git_fixture!()
    git!(fixture.repo, ["update-ref", "-d", "refs/remotes/origin/0.1.x"])

    {missing_output, missing_status} =
      command(
        "sh",
        [@script, "release-candidate/missing-ref", fixture.base, fixture.f1, fixture.f2],
        cd: fixture.repo
      )
    ...
    assert missing_output ==
             "release candidate: refs/remotes/origin/0.1.x is missing; fetch origin/0.1.x first.\n"

    refute git_ref?(fixture.repo, "refs/heads/release-candidate/missing-ref")
    assert git!(fixture.repo, ["rev-parse", "HEAD"]) == fixture.f2
  end
