defmodule Tightbeam.RestCoreDetailRoutesTest do
  ...
  test "real reopen returns the canonical REST assignment and one notice", ctx do
    call = %{
      verb: "revoke-assignment",
      origin: "user:flynn",
      principal: {:user, "flynn"},
      session_key: nil,
      params: %{assignment_id: ctx.assignment.id, reason: "first REST close"}
    }

    closed = Assignments.__handle__(ctx.db, "revoke-assignment", call)
    assert closed.state == "closed"
    assert closed.revocationReason == "first REST close"
    closed_notice = Publisher.state_notice(ctx.db, call, closed)

    assert JSON.decode!(get(ctx, "/api/assignments/#{ctx.assignment.id}").resp_body)["item"] ==
             closed_notice["payload"]

    :ok = Hub.register(Hub, self(), %{mode: :all, db: ctx.db, user_id: "flynn", is_admin: true})

    handlers =
      Tightbeam.Gateway.handlers(%{db: ctx.db, wake_tick_ms: 60_000, base_dir: System.tmp_dir!()})

    reopen = %{
      call
      | verb: "reopen-assignment",
        params: %{assignment_id: ctx.assignment.id, reason: "REST parity"}
    }

    assert {:ok, %{state: "open"}} = Tightbeam.Dispatch.dispatch(ctx.db, handlers, reopen)
    assert_receive {:firehose_notice, %{"class" => "wake.canceled", "payload" => canceled}}
    assert canceled["workItemId"] == ctx.work_item.id
    ...
    assert_receive {:firehose_notice,
                    %{"class" => "assignment.reopened", "payload" => payload, "refs" => refs}}

    assert refs["assignmentId"] == ctx.assignment.id
    response = get(ctx, "/api/assignments/#{ctx.assignment.id}")
    assert response.status == 200
    assert JSON.decode!(response.resp_body)["item"] == payload
    assert payload["revocationReason"] == nil
    notice = %{"class" => "assignment.reopened", "payload" => payload, "refs" => refs}
    assert StateVisibility.visible?(ctx.db, notice, "flynn", false)
    refute StateVisibility.visible?(ctx.db, notice, "unrelated-user", false)
    assert StateVisibility.visible?(ctx.db, notice, "admin", true)
    assert get(ctx, "/api/assignments/#{ctx.assignment.id}", "invalid-token").status == 401

    assert [%{priorOutcome: "revoked", reason: "REST parity"}] =
             Assignments.list_reopenings(ctx.db, ctx.assignment.id)

    second_call = put_in(call, [:params, :reason], "second REST close")
    second = Assignments.__handle__(ctx.db, "revoke-assignment", second_call)
    assert second.revocationReason == "second REST close"
    second_notice = Publisher.state_notice(ctx.db, second_call, second)

    assert JSON.decode!(get(ctx, "/api/assignments/#{ctx.assignment.id}").resp_body)["item"] ==
             second_notice["payload"]

    assert second_notice["payload"]["rowVersion"] > payload["rowVersion"]
    assert Assignments.__handle__(ctx.db, "revoke-assignment", second_call) == second
  end
  ...
  test "artifact archive and release notices equal the current ordered REST detail", ctx do
    ...
    File.write!(Path.join(workspace, "reports/core-detail.md"), "retained artifact")
    ...
    :ok = Hub.register(Hub, self(), %{mode: :all, db: ctx.db, user_id: "flynn", is_admin: true})
    :ok = Artifacts.archive_session(ctx.db, ctx.admin_session.session_key, workspace, archive)
    assert_receive {:firehose_notice, %{"class" => "artifact.archived", "payload" => archived}}

    for {class, expected} <- [{"artifact.archived", archived}, {"artifact.released", nil}] do
      payload =
        if class == "artifact.released" do
          :ok = Hub.delivered(Hub, self())
          assert Artifacts.release(ctx.db, ctx.artifact.artifact_id).state == "released"

          assert_receive {:firehose_notice,
                          %{"class" => "artifact.released", "payload" => released}}
      ...
      response = get(ctx, "/api/artifacts/#{ctx.artifact.artifact_id}")
      assert response.status == 200
      bytes = StateResources.encode_item("artifacts", payload, ctx.catalog)
      assert response.resp_body == ~s({"schemaVersion":1,"resource":"artifacts","item":#{bytes}})
    end
  end

  test "real agent return advances ordered REST-notice version and retry preserves it", ctx do
    target = ensure_main_session(ctx.db, "return-reader")
    {_port, ws} = start_change_server(ctx, ["decision_request.returned"])

    ask_call =
      firehose_call("ask", target.session_key, %{question: "Which boundary?"})
      |> Map.merge(%{
    ...
    request = Escalation.ask(ctx.db, ask_call)
    {opened_raw, ws} = receive_raw_change(ws, "decision_request.opened")
    opened = JSON.decode!(opened_raw)["payload"]

    assert Map.take(opened, ~w(returnedBy returnReason returnedAt)) ==
             %{"returnedBy" => nil, "returnReason" => nil, "returnedAt" => nil}

    return_call = %{
      verb: "return",
      origin: "agent:#{target.session_key}",
      principal: {:session, target.session_key},
      session_key: target.session_key,
      firehose_in_txn: true,
      firehose_hub: Hub,
      params: %{request: request.id, reason: "name the predecessor"}
    }

    returned = Escalation.return_request(ctx.db, return_call)
    assert returned.status == "returned"
    {returned_raw, ws} = receive_raw_change(ws, "decision_request.returned")
    notice = JSON.decode!(returned_raw)
    response = get(ctx, "/api/decision-requests/#{request.id}")
    assert response.status == 200
    item = JSON.decode!(response.resp_body)["item"]
    assert item == notice["payload"]
    assert raw_json_value(returned_raw, "payload") == raw_json_value(response.resp_body, "item")
    assert item["returnedBy"] == returned.returned_by
    assert item["returnReason"] == returned.return_reason
    assert item["returnedAt"] == returned.returned_at
    assert item["rowVersion"] == 2
    assert item["rowVersion"] > opened["rowVersion"]

    assert_ordered_fields(
      raw_json_value(response.resp_body, "item"),
      Enum.drop(@r7_fields["decision requests"], -1) ++
        ~w(returnedBy returnReason returnedAt rowVersion)
    )

    assert Escalation.return_request(ctx.db, return_call) == returned
    retry_response = get(ctx, "/api/decision-requests/#{request.id}")
    assert retry_response.resp_body == response.resp_body

    assert Enum.count(Tightbeam.EventLog.lifecycle_events(ctx.db), fn event ->
  ...
  test "B2 collection shares detail bytes and binds pagination to filters and principal", ctx do
    for priority <- [2, 7] do
      assert %{id: _} =
               WorkItems.__handle__(ctx.db, "work-item-create", %{
                 ...
                 principal: {:user, "flynn"},
                 session_key: nil,
                 params: %{title: "B2 priority #{priority}", priority: priority}
               })
    end

    all = get(ctx, "/api/work-items?state=open")
    assert all.status == 200
    assert %{"resource" => "work items", "items" => items} = JSON.decode!(all.resp_body)
    assert length(items) == 3
    assert Enum.sort(Enum.map(items, & &1["priority"])) == [2, 4, 7]

    assert Enum.map(items, &[&1["createdAt"], &1["id"]]) ==
             Enum.sort(Enum.map(items, &[&1["createdAt"], &1["id"]]))

    for item <- items do
      detail = get(ctx, "/api/work-items/" <> item["id"])
      assert detail.status == 200
      assert JSON.decode!(detail.resp_body)["item"] == item
    end

    latest = get(ctx, "/api/work-items?state=open&limit=1")
    assert latest.status == 200
    page = JSON.decode!(latest.resp_body)
    assert length(page["items"]) == 1
    assert page["page"]["hasMoreBefore"]
    cursor = page["page"]["oldestCursor"]
    prior = get(ctx, "/api/work-items?state=open&limit=2&before=" <> URI.encode_www_form(cursor))
    assert prior.status == 200
    assert JSON.decode!(prior.resp_body)["items"] ++ page["items"] == items
    mismatch = get(ctx, "/api/work-items?state=closed&before=" <> URI.encode_www_form(cursor))
    assert mismatch.status == 400
    assert JSON.decode!(mismatch.resp_body)["error"]["code"] == "invalid_cursor"

    for suffix <- ["state=bogus", "isBug=bogus", "unknown=x", "limit=0"] do
      response = get(ctx, "/api/work-items?" <> suffix)
      assert response.status == 400
    end

    unauthorized = get(ctx, "/api/work-items?unknown=x", "bad-token")
    assert unauthorized.status == 401
    assert JSON.decode!(unauthorized.resp_body)["error"]["code"] == "auth_failed"
    empty = get(ctx, "/api/work-items?ownerUserId=absent-owner")
    assert empty.status == 200
    assert JSON.decode!(empty.resp_body)["items"] == []
  end

  @tag rest_r7_boundary: true
  test "B2 collection enforces principal resolution and rejects cross-principal cursors", ctx do
    outsider = outsider_device(ctx)
    path = "/api/work-items?limit=1"
    page = get(ctx, path)
    assert page.status == 200
    cursor = JSON.decode!(page.resp_body)["page"]["oldestCursor"]
    assert is_binary(cursor)
    before = URI.encode_www_form(cursor)

    own = get(ctx, "/api/work-items?before=" <> before)
    assert own.status == 200

    other = get(ctx, path, outsider.token)
    assert other.status == 200
    assert JSON.decode!(other.resp_body)["items"] == []
    cross = get(ctx, "/api/work-items?before=" <> before, outsider.token)
    assert_error(cross, 400, "work items", "invalid_cursor")

    session_page = get(ctx, path, ctx.admin_session.cli_token)
    assert session_page.status == 200

    assert Enum.map(JSON.decode!(session_page.resp_body)["items"], & &1["id"]) == [
             ctx.work_item.id
           ]

    cross_session = get(ctx, "/api/work-items?before=" <> before, ctx.admin_session.cli_token)
    assert_error(cross_session, 400, "work items", "invalid_cursor")

    org_page = get(ctx, path <> "&asUser=flynn", "tbc_core_detail")
    assert org_page.status == 200
    assert JSON.decode!(org_page.resp_body)["items"] == JSON.decode!(page.resp_body)["items"]

    for {suffix, token, status, code} <- [
          {"?unknown=x", "bad-token", 401, "auth_failed"},
          {"", "tbc_core_detail", 400, "invalid_message"},
          {"?asUser=mallory", ctx.admin_session.cli_token, 403, "identity_not_yours"},
          {"?asUser=flynn", ctx.token, 400, "invalid_as_user"},
          {"?asUser=flynn&asUser=flynn", "tbc_core_detail", 400, "invalid_as_user"},
          {"?asUser=flynn%zz", ctx.token, 400, "malformed_query"}
        ] do
  ...
  defp start_change_server(ctx, extra_classes \\ []) do
    bandit =
      start_supervised!(
        {Bandit,
