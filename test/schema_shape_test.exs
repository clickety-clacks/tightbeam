...
defmodule Tightbeam.SchemaShapeTest do
  ...
  test "the exact v15 predecessor gains the premise gate", %{db: db} do
    assert :ok = Schema.ensure_all(db)
  ...
  test "the premise gate upgrade refuses a predecessor that is not v15", %{db: db} do
    assert :ok = Schema.ensure_all(db)
    drop_premise_gate(db)
  ...
  test "a v15 database is refused by name on boot", %{db: db} do
    assert :ok = Schema.ensure_all(db)
    :ok = DB.execute(db, "UPDATE schema_stamp SET shape='coordination-fabric-v1-phase1-v15'")
