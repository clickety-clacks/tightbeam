defmodule Tightbeam.StaleTurnSettlementTest do
  ...
  setup do
    ...
    :ok =
      ...
      VALUES ('mike',1,1),('other-admin',1,1),
             ('reader',0,1);
  ...
  test "lane-live, provider-live, and uncorrelatable targets are zero-mutation refusals", ctx do
    ...
    pending =
      Task.async(fn ->
        Conn.request(conn, "session/prompt", %{sessionId: "hs-2", prompt: []},
          timeout: :infinity,
          session_id: "hs-2"
        )
      end)
  ...
  test "gateway owner loss releases the fence and boot recovery alone classifies the turn", ctx do
    ...
    # A completed coordinator call proves its blocked fence callback released.
    # Counting a nonexistent entry field would pass before any release.
    assert {:ok, adapter, 1} =
  ...
  defp stale_turn!(db, session_key, prompt, request_id) do
    ...
    assert :ok = Ledger.stamp_adapter(db, seq, 1)
    ...
    {:ok, [[status, error, lifecycle, published]]} =
      DB.query(
        ...
        SELECT t.status,t.error,
          ...
          CASE WHEN t.publishedAt IS NULL THEN 0 ELSE 1 END
