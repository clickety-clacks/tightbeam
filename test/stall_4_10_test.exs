defmodule Tightbeam.Stall410Test do
  ...
  test "v13 horizon migration keeps declared history and enables terminal cancellation", ctx do
    item = create(ctx.db, {:user, "mike"}, "migrated goal")
    horizon = boundary(ctx.db, item.id, "legacy boundary", 1_000, "legacy-horizon")

    :ok = DB.execute(ctx.db, "ALTER TABLE work_item_horizons RENAME TO work_item_horizons_v14")
    ...
    :ok =
      DB.execute(
        ...
        FROM work_item_horizons_v14
        """
      )

    :ok = DB.execute(ctx.db, "DROP TABLE work_item_horizons_v14")

    :ok =
      DB.execute(
        ctx.db,
        "UPDATE schema_stamp SET shape='coordination-fabric-v1-phase1-v13'"
      )
