defmodule Tightbeam.AttentionTierFixture do
  @moduledoc """
  ...
  def run_case!(scenario, _base, _locks) do
    Tightbeam.GuardGatewayFixture.run!(fn %{db: db, base: base_dir, config: guarded} ->
      ...
      try do
        ...
        :ok =
          DB.execute(db, "INSERT INTO users (userId, isAdmin, createdAt) VALUES ('flynn',0,1)")
        ...
        :sys.replace_state(catalog, fn state ->
          ...
          %{
            state
            | hosts: fn -> %{"testhost" => %{base_dir: base_dir, ssh: nil}} end,
        ...
        scenario(scenario, %{db: db, config: config, base_dir: base_dir})
      after
        if Process.alive?(fixtures), do: Supervisor.stop(fixtures)
      end
    end)

    IO.puts("attention_tier-case: #{scenario}: ok")
  end
  ...
  def run!(tmp, scenario) do
    %{executable: executable, args: args, env: env} =
      Tightbeam.GuardRuntimeFixture.prepare!(tmp, "attention_tier_runtime.exs")
    ...
    assert output =~ "attention_tier-case: #{scenario}: ok"
  end
