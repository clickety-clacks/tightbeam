...
try do
  ...
  assert is_binary(first["cliToken"]) and first["cliToken"] != ""
  assert second["port"] == 5_432
  assert File.stat!(Path.join(base, "gateway.json")).mode |> Bitwise.band(0o777) == 0o600
...
IO.puts("guarded-gateway-token: ok")
