defmodule Tightbeam.ToplinesTest do
  ...
  test "proof 14: a dispatched read writes only the ordinary audit row", ctx do
    ...
    handlers = %{
      "toplines" => fn call -> Toplines.roster(ctx.db, call) end,
      "topline" => fn call -> Toplines.topline(ctx.db, call) end
    }
  ...
  defp read(ctx, kind, params, opts \\ []) do
    ...
    case kind do
      :roster -> Toplines.roster(ctx.db, call)
      :topline -> Toplines.topline(ctx.db, call)
    end
  end
